...
resource "azurerm_linux_web_app" "mydockerapp" {
  ...
  app_settings = {
    ...
    DOCKER_REGISTRY_SERVER_URL          = module.container_registry.my_webapp_acr_login_server
    DOCKER_REGISTRY_SERVER_USERNAME     = module.container_registry.my_webapp_acr_admin_username
    DOCKER_REGISTRY_SERVER_PASSWORD     = module.container_registry.my_webapp_acr_admin_password
  }

  site_config {
    application_stack {
      docker_image     = "${module.container_registry.my_webapp_acr_login_server}/${var.docker_image_name}"
